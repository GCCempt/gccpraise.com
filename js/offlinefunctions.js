...
function printSetlist()
{
	...
	{	
		...
		for (i = 0; i <= slist.length-1; i++)
		{
			...
			$('#setlist').append('<span id="' + slist[i].replace(/ /g, "_") + '-' + i +'" class="' + class + '">' +
			'<a href="' + slist[i] + '.html" onclick="return setSong(' + i + ');">' + slist[i] + 
		...
		listOptions = "| <a href='" + slist[0] + 
		".html' onclick='return setSong(0);'>Start</a> | " + "<a href='" + slist[csong] + ".html'>Continue</a>"
	...
	if (curpage.toUpperCase().indexOf('INDEX') == -1 && curpage.charAt(curpage.length-1)!='/' &&
		curpage.toUpperCase().indexOf('STATIC') == -1)
	{
		updateSongNav();
...
function updateSongNav()
{
	...
	if (csong > 0)
	{
		$('#songnav').append("<a href='" + slist[csong-1] + ".html' onclick='prev();'>Previous Song</a> | ");
	}
	if (csong < slist.length-1)
	{
		$('#songnav').append("<a href='" + slist[csong+1] + ".html' onclick='next();'>Next Song</a>");
	}
