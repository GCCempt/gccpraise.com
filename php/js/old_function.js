...
function printSetlist()
{
	...
	{	
		...
		for (i = 0; i <= slist.length-1; i++)
		{
			...
			'<a href="song.php?s=' + slist[i] + '" onclick="return setSong(' + i + ');">' + slist[i] + 
		...
		listOptions = "| <a href='song.php?s=" + slist[0] + 
		"' onclick='return setSong(0);'>Start</a> | " + "<a href='song.php?s=" + slist[csong] + "'>Continue</a>"
...
function updateSongNav()
{
	...
	if (csong > 0)
	{
		$('#songnav').append("<a href='song.php?s=" + slist[csong-1] + "' onclick='prev();'>Previous Song</a> | ");
	}
	if (csong < slist.length-1)
	{
		$('#songnav').append("<a href='song.php?s=" + slist[csong+1] + "' onclick='next();'>Next Song</a>");
	}
